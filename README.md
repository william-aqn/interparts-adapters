...
│   ├── rockauto/                  # www.rockauto.com (mode: http, no auth)
│   └── worldpac-speeddial/        # speeddial.worldpac.com (mode: browser, persistent session)
├── shared/
│   ├── interfaces/
...
| **Новая** adapter-директория появилась на хосте (git pull, ручное копирование) | **Нет.** Нажми **«Sync from disk»** на странице Adapters в админке (`POST /api/adapters/sync`) — оркестратор перечитает `adapters/*/meta.json` и создаст записи в Mongo для новых адаптеров. Существующие записи не трогаются. |
